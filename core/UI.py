# -*- coding: utf-8 -*-
"""
Visor GLB + VISEMAS por TEXTO (sin audio) + Panel CRM en ventana aparte.

- Ventana 1 (Panda3D): Nacho 3D con visemas y lenguaje corporal.
- Ventana 2 (Tkinter): Panel CRM de prospecto, se actualiza vía:
      update_crm({...}) o HTTP GET /crm?nombre=...&empresa=...&...
"""
...
# ---------- COLORES PARA LA VENTANA CRM (Tkinter) ----------
CRM_BG = "#015666"         # fondo externo azul petróleo
CRM_CARD_BG = "#FFFFFF"    # tarjeta blanca
CRM_LABEL_FG = "#555555"   # etiquetas grises
CRM_VALUE_FG = "#222222"   # texto normal
CRM_EMAIL_FG = "#C62828"   # email en rojo
CRM_TITLE_FG = "#222222"   # título

# Referencias globales
APP_INSTANCE = None  # instancia global del visor Panda3D
CRM_PANEL = None     # instancia global del panel CRM (Tkinter)
...
class CRMPanel:
    """
    Ventana CRM separada, hecha con Tkinter.
    Muestra:
      - Nombre
      - Empresa
      - Correo
    ...
    Ocupa todo el tamaño de la ventana (el usuario puede redimensionar y mover).
    """
    ...
    def __init__(self):
        ...
        self.root = tk.Tk()
        self.root.title("Panel CRM - Nacho Totem")
        # Tamaño inicial; el usuario puede redimensionar
        self.root.geometry("700x400")
        self.root.configure(bg=CRM_BG)
        ...
        style.configure("Card.TFrame", background=CRM_CARD_BG)
        style.configure(
            "Title.TLabel",
            background=CRM_CARD_BG,
            foreground=CRM_TITLE_FG,
            font=("Segoe UI", 14, "bold"),
        )
        style.configure(
            "FieldLabel.TLabel",
            background=CRM_CARD_BG,
            foreground=CRM_LABEL_FG,
            font=("Segoe UI", 10),
        )
        style.configure(
            "FieldValue.TLabel",
            background=CRM_CARD_BG,
            foreground=CRM_VALUE_FG,
            font=("Segoe UI", 11),
        )
        style.configure(
            "EmailValue.TLabel",
            background=CRM_CARD_BG,
            foreground=CRM_EMAIL_FG,
            font=("Segoe UI", 11, "bold"),
        )

        # --- LAYOUT PRINCIPAL ---
        outer = tk.Frame(self.root, bg=CRM_BG)
        ...
        title_frame.pack(fill="x", padx=24, pady=(18, 10))
        ...
        self.vars = {
            ...
            "empresa": tk.StringVar(value="-"),
            "correo": tk.StringVar(value="-"),
            "telefono": tk.StringVar(value="-"),
            "diagnostico": tk.StringVar(value="-"),
        }

        fields_frame = ttk.Frame(card, style="Card.TFrame")
        fields_frame.pack(fill="both", expand=True, padx=24, pady=(0, 24))
        ...
        for r, (label_text, key) in enumerate(rows):
            ...
            style_name = "EmailValue.TLabel" if key == "correo" else "FieldValue.TLabel"
            ...
            val.grid(row=r, column=1, sticky="w", pady=6)
    ...
    def update_fields(self, data: dict):
        """
        Recibe un dict y actualiza la ventana CRM.
        ...
        # Aseguramos que la actualización corra en el hilo del mainloop
...
class TextVisemeDemo(ShowBase):
    def __init__(self, glb_path, bg_path=None):
        ...
        try:
            ...
            print(
                f"❌ Error al cargar modelo '{glb_path}': {e}\n"
                f"(Instala: pip install panda3d-gltf)"
            )
            sys.exit(1)
        ...
        # HUD (texto arriba izquierda) — lo creamos pero lo dejamos oculto
        self.ui_font = None
        ...
        self.status_node.setPos(-self.getAspectRatio() + 0.1, 0.9, 0.9)

        # 👇 Esto lo oculta completamente (ya no verás texto de visemas ni nada)
        self.status_node.hide()
        ...
        self.brow_L = self._find(
            ["brow.B.L", "brow.T.L", "brow.B.L.001", "brow.T.L.001"]
        )
        self.brow_R = self._find(
            ["brow.B.R", "brow.T.R", "brow.B.R.001", "brow.T.R.001"]
        )
        ...
        print("\n🧠 Nacho listo.")
        print("Puedes llamarlo desde otro módulo usando speak(text) o via HTTP.\n")
    ...
    def _setup_background_card(self, image_path: str):
        ...
        print(
            f"🖼 Fondo activo: {image_path} "
            f"({self.bg_tex.getXSize()}x{self.bg_tex.getYSize()})"
        )
    ...
    def _apply_params(self, params, open_amount):
        ...
        dz_up = clamp(
            self.lip_z_gain * top_v,
            -self.max_lip_dz,
            self.max_lip_dz,
        ) * lip_close_mult
        dz_down = clamp(
            self.lip_z_gain * bot_v,
            -self.max_lip_dz,
            self.max_lip_dz,
        ) * lip_close_mult
        ...
        jaw_dz = s * clamp(
            self.jaw_z_gain * (bot_v * jaw_open),
            -self.max_jaw_dz,
            self.max_jaw_dz,
        )
        ...
        close_amt = s * clamp(
            self.close_dz_gain * shape_amt * close_k,
            0.0,
            self.max_close_dz,
        )
    ...
    def _update_lids_translate_zx(self, t):
        ...
        for (name, j, w) in self.lid_top_L:
            ...
            dz = -1.0 * clamp(
                (0.0032 * blink_amt * w) * s + micro_z * w,
                -0.0060 * s,
                0.0060 * s,
            )
            dx = -1.0 * clamp(
                (0.0020 * lat * w) * s + micro_x * w,
                -0.0030 * s,
                0.0030 * s,
            )
        ...
        for (name, j, w) in self.lid_bot_L:
            bpx, bpy, bpz = self.base_pos[name]
            dz = +1.0 * clamp(
                (0.0016 * blink_amt * w) * s + 0.5 * micro_z * w,
                -0.0035 * s,
                0.0035 * s,
            )
            dx = -1.0 * clamp(
                (0.0012 * lat * w) * s + 0.5 * micro_x * w,
                -0.0020 * s,
                0.0020 * s,
            )
            j.setPos(bpx + dx, bpy, bpz + dz)
    ...
    def _update_body_language(self, t: float, speak_level: float):
        ...
        - Brazos y antebrazos siguen usando eje principal P (pitch).
        """
        ...
        ARM_RANGE = 0.70          # amplitud moderada
        FOREARM_SPEED = 0.05
        ...
        head_turn = MOVEMENT_FLIP * head_turn_amp * math.sin(
            head_t * 0.8 + 1.5
        )
        ...
        arm_wave_L = MOVEMENT_FLIP * clamp(
            arm_wave_scalar_L * l_factor + arm_noise_L, -1.0, 1.0
        )
        arm_wave_R = MOVEMENT_FLIP * clamp(
            arm_wave_scalar_R * r_factor + arm_noise_R, -1.0, 1.0
        )
        ...
        for side, wave, hand in (
            ...
            for name, j in hand:
                ...
                twist_noise = (
                    2.0
                    * s
                    * math.sin(small_t + (0.5 if side == "L" else -0.5))
                ) * ARM_RANGE
...
def update_crm(lead_data: dict) -> bool:
    """
    Actualiza la ventana CRM (Tkinter).
    ...
    Alias aceptados:
        name, company, email, proposal, phone/Phone
    """
    global CRM_PANEL
    if CRM_PANEL is None:
        print("⚠ El panel CRM aún no está inicializado.")
        return False

    CRM_PANEL.update_fields(lead_data or {})
    return True
    ...
    def do_GET(self):
        ...
        if path.startswith("/crm"):
            data = {
                ...
                or qs.get("phone", [""])[0]
                or qs.get("Phone", [""])[0],
                "diagnostico": qs.get("diagnostico", [""])[0]
                or qs.get("proposal", [""])[0],
            }
            ...
            if ok:
                ...
                self._send_response(503, "Panel CRM aún no está listo.")
            return
        ...
        if not text:
            ...
            self._send_response(
                503, "Nacho aún no está listo (APP_INSTANCE es None)."
            )
        else:
...
def start_http_server(port: int = HTTP_PORT):
    ...
    print(
        "   CRM:      http://localhost:7000/crm?nombre=Lulu&empresa=Ei3"
        "&correo=test%40mail.com&telefono=5551234567&diagnostico=Interesada"
    )
    server.serve_forever()


def start_crm_window():
    global CRM_PANEL
    CRM_PANEL = CRMPanel()
    CRM_PANEL.run()
...
if __name__ == "__main__":
    APP_INSTANCE = TextVisemeDemo(GLB_PATH, bg_path=BG_PATH)

    # Hilo para input por consola (texto -> visemas)
    t = threading.Thread(target=APP_INSTANCE._input_thread, daemon=True)
    t.start()
    ...
    # Hilo para ventana CRM (Tkinter)
    crm_thread = threading.Thread(target=start_crm_window, daemon=True)
    crm_thread.start()

    # Bucle principal de Panda3D
    APP_INSTANCE.run()

...
class TextVisemeDemo(ShowBase):
    def __init__(self, glb_path, bg_path=None):
        ...
        # Brazos completos + columna / cabeza para lenguaje corporal
        ...
        # Offset aleatorio para que no se vea tan robótico
        self._gesture_offset = random.uniform(0.0, math.pi * 2.0)
        # Estado interno de lenguaje corporal
        self._gesture_mode = "REST"      # REST, OPEN_PALMS, STEADY_R, STEADY_L
        self._gesture_mode_t = 0.0       # hasta cuándo dura el gesto actual
        self._last_gesture_mode = "REST" # para no repetir el mismo todo el tiempo

        # Bases de posición / rotación (incluyendo columna, cabeza, brazos completos)
        self.base_hpr = {}
        self.base_pos = {}
        for group in [
    ...
    def _text_to_viseme_sequence(self, text: str):
        ...
        speed_factor = 2.35  # 2x más rápido aprox
    ...
    def _update_body_language(self, t: float, speak_level: float):
        ...
        - En reposo: respiración suave.
        - Hablando: gestos de confianza (pecho abierto, manos visibles).
        - TODO lo que es lado L se mueve en sentido contrario al que se estaba moviendo.
        - Se mueven hombro, codo y muñeca.
        """
        s = self.MOVEMENT_SCALE
        speak_level = clamp(speak_level, 0.0, 1.0)
        now = t

        # ---------- MODO / ESTADO DE GESTO (NO REPETITIVO) ----------
        if not hasattr(self, "_gesture_mode"):
            self._gesture_mode = "REST"
            self._gesture_mode_t = 0.0
            self._last_gesture_mode = "REST"

        # Si casi no está hablando, volvemos a REST
        if speak_level < 0.18:
            self._gesture_mode = "REST"
        else:
            # Si se acabó el tiempo del gesto actual, elegimos otro
            if now >= self._gesture_mode_t:
                modes = ["OPEN_PALMS", "STEADY_R", "STEADY_L"]
                # evitar repetir exactamente el mismo modo
                ...
                # cada gesto dura ~1.4–2.6 segundos
                self._gesture_mode_t = now + random.uniform(1.4, 2.6)

        mode = self._gesture_mode

        # ---------- TIEMPOS (LENTOS, ESTILO NATURAL) ----------
        body_t    = t * 0.14 + self._gesture_offset
        head_t    = t * (0.18 + 0.22 * speak_level) + self._gesture_offset * 0.3
        gesture_t = t * (0.22 + 0.30 * speak_level) + self._gesture_offset * 0.7
        small_t   = t * 0.55

        # ---------- COLUMNA / PESO DEL CUERPO ----------
        sway_yaw   = 1.5 * s * math.sin(body_t)
        sway_pitch = 1.2 * s * math.sin(body_t * 0.7 + 0.8)
        lift       = 0.003 * s * math.sin(body_t * 0.9)
        ...
        # ---------- CABEZA (ASENTIR SUAVE Y MIRADA SEGURA) ----------
        head_nod  = (2.0 + 2.5 * speak_level) * s * math.sin(head_t)
        head_turn = (1.5 * speak_level) * s * math.sin(head_t * 0.8 + 1.5)

        for name, j in self.head_bones:
            bh, bp, br = self.base_hpr[name]
            j.setHpr(bh + head_turn, bp + head_nod, br)

        # ---------- PECHO ABIERTO / RESPIRACIÓN ----------
        chest_open = (4.0 + 4.0 * speak_level) * s    # pecho se abre = confianza
        breath     = (1.5 + speak_level) * s * math.sin(body_t * 0.9)

        # amplitud de gestos al hablar (controlada)
        talk_amp = (4.0 + 6.0 * speak_level) * s

        phase_main = gesture_t
        phase_alt  = gesture_t + math.pi * 0.7
        ...
        if mode == "REST":
            ...
            l_factor = r_factor = 0.25

        # Olas base (sin invertir todavía)
        wave_L_raw = math.sin(phase_alt)  * l_factor
        wave_R_raw = math.sin(phase_main) * r_factor

        # Ruido pequeño para evitar que se vea periódico
        noise_L = 0.15 * math.sin(small_t * 1.3 + 0.5)
        noise_R = 0.15 * math.sin(small_t * 1.1 + 2.0)
        wave_L_raw += noise_L
        wave_R_raw += noise_R

        wave_L_raw = clamp(wave_L_raw, -1.0, 1.0)
        wave_R_raw = clamp(wave_R_raw, -1.0, 1.0)

        # 🔁 AQUÍ INVERTIMOS EL LADO IZQUIERDO (L) PARA QUE SE MUEVA AL LADO CONTRARIO
        wave_L = -wave_L_raw
        wave_R = wave_R_raw

        # ---------- HOMBROS / BRAZOS SUPERIORES ----------
        for side, upper_arm, base_rot, wave in (
            ...
            for name, j in upper_arm:
                ...
                # dP = subir/bajar brazo, dR = abrir pecho / rotar codo hacia fuera
                dP = breath * 0.4 + talk_amp * 0.5 * wave
                dR = chest_open * 0.4 + talk_amp * 0.3 * wave
        ...
        # ---------- ANTEBRAZOS (CODO) ----------
        # Más amplitud para que se note el codo, pero suave
        for side, wave, forearm in (
            ("L", wave_L, self.forearm_L),
            ("R", wave_R, self.forearm_R),
        ):
            for name, j in forearm:
                bh, bp, br = self.base_hpr[name]
                dP = talk_amp * 0.45 * wave   # inclinación del codo
                dR = talk_amp * 0.28 * wave   # giro suave
                j.setHpr(bh, bp + dP, br + dR)

        # ---------- MANOS / MUÑECA ----------
        for side, wave, hand in (
            ("L", wave_L, self.hand_L),
            ("R", wave_R, self.hand_R),
        ):
            for name, j in hand:
                bh, bp, br = self.base_hpr[name]

                # roll base = mostrar palmas → confianza
                base_roll_extra = 6.0 * s
                if speak_level > 0.25:
                    base_roll_extra += 4.0 * s
                if mode == "OPEN_PALMS":
                    base_roll_extra += 6.0 * s  # manos aún más abiertas

                # leve balanceo con el gesto + micro-giro continuo
                dP = talk_amp * 0.28 * wave
                twist_noise = 4.0 * s * math.sin(small_t + (0.5 if side == "L" else -0.5))

                j.setHpr(
                    bh,
                    bp + dP,
                    br + base_roll_extra + twist_noise,
                )
    ...
    def _animate(self, task):
        ...
        # Lenguaje corporal (columna, cabeza, brazos)
...
# ---------- NUEVO: SERVIDOR HTTP EN PUERTO 7000 ----------
class NachoRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        # Prioridad: parámetro ?t=texto
        if "t" in qs and qs["t"]:
            text = qs["t"][0]
        else:
            # Si no hay ?t=..., tomamos el PATH: /Hola%20Mundo -> "Hola Mundo"
            text = unquote(parsed.path.lstrip("/"))

        text = (text or "").strip()

        if not text:
            self._send_response(400, "Debes enviar texto en la URL, ej: /Hola%20soy%20Nacho o ?t=Hola")
            return
...
# ---------- MODO STANDALONE (con input por consola + HTTP) ----------
if __name__ == "__main__":
    ...
    # Hilo de input por consola (opcional)
